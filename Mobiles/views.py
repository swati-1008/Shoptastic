...
def oneplus(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB1').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    word_comments = Comments.objects.filter(pid = 'MOB1').exclude(comment = 'NULL')
...
def redmi9power(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB2').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    rating_sum = rate1 * 1 + rate2 * 2 + rate3 * 3 + rate4 * 4 + rate5 * 5
    total_ratings = rate1 + rate2 + rate3 + rate4 + rate5
    word_comments = Comments.objects.filter(pid = 'MOB2').exclude(comment = 'NULL')
    total_comments = len(word_comments)
    # for i in word_comments:
    #     print(i.comment)
    final_rating = round((rating_sum / total_ratings), 2)
    prev_model_rating = Mobiles.objects.get(product_id='MOB2').model_rating
    if rating != 0:
        model_rating = round(((prev_model_rating * total_comments) + rating) / (total_comments + 1), 2)
    else:
        model_rating = prev_model_rating
    obj, created = Mobiles.objects.update_or_create(product_id='MOB2', defaults={'average_customer_rating':final_rating, 'model_rating': model_rating})
    obj.save()
    rating1 = (rate1 / (total_ratings)) * 100
    rating2 = (rate2 / (total_ratings)) * 100
    rating3 = (rate3 / (total_ratings)) * 100
    ...
    final_star = final_rating * 20                          # () * 100 / 5 = () * 20
    model_star = model_rating * 20
    comments = Comments.objects.filter(pid='MOB2')
...
def samsunggalaxym51(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB3').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    rating_sum = rate1 * 1 + rate2 * 2 + rate3 * 3 + rate4 * 4 + rate5 * 5
    total_ratings = rate1 + rate2 + rate3 + rate4 + rate5
    word_comments = Comments.objects.filter(pid = 'MOB3').exclude(comment = 'NULL')
    total_comments = len(word_comments)
    # for i in word_comments:
    #     print(i.comment)
    final_rating = round((rating_sum / total_ratings), 2)
    prev_model_rating = Mobiles.objects.get(product_id='MOB3').model_rating
    if rating != 0:
        model_rating = round(((prev_model_rating * total_comments) + rating) / (total_comments + 1), 2)
    else:
        model_rating = prev_model_rating
    obj, created = Mobiles.objects.update_or_create(product_id='MOB3', defaults={'average_customer_rating':final_rating, 'model_rating': model_rating})
    obj.save()
    rating1 = (rate1 / (total_ratings)) * 100
    rating2 = (rate2 / (total_ratings)) * 100
    rating3 = (rate3 / (total_ratings)) * 100
    ...
    final_star = final_rating * 20                          # () * 100 / 5 = () * 20
    model_star = model_rating * 20
    comments = Comments.objects.filter(pid='MOB3')
...
def iphone12mini(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB6').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    rating_sum = rate1 * 1 + rate2 * 2 + rate3 * 3 + rate4 * 4 + rate5 * 5
    total_ratings = rate1 + rate2 + rate3 + rate4 + rate5
    word_comments = Comments.objects.filter(pid = 'MOB6').exclude(comment = 'NULL')
    total_comments = len(word_comments)
    # for i in word_comments:
    #     print(i.comment)
    final_rating = round((rating_sum / total_ratings), 2)
    prev_model_rating = Mobiles.objects.get(product_id='MOB6').model_rating
    if rating != 0:
        model_rating = round(((prev_model_rating * total_comments) + rating) / (total_comments + 1), 2)
    else:
        model_rating = prev_model_rating
    obj, created = Mobiles.objects.update_or_create(product_id='MOB6', defaults={'average_customer_rating':final_rating, 'model_rating': model_rating})
    obj.save()
    rating1 = (rate1 / (total_ratings)) * 100
    rating2 = (rate2 / (total_ratings)) * 100
    rating3 = (rate3 / (total_ratings)) * 100
    ...
    final_star = final_rating * 20                          # () * 100 / 5 = () * 20
    model_star = model_rating * 20
    comments = Comments.objects.filter(pid='MOB6')
...
def nokia34(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB4').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    rating_sum = rate1 * 1 + rate2 * 2 + rate3 * 3 + rate4 * 4 + rate5 * 5
    total_ratings = rate1 + rate2 + rate3 + rate4 + rate5
    word_comments = Comments.objects.filter(pid = 'MOB4').exclude(comment = 'NULL')
    total_comments = len(word_comments)
    # for i in word_comments:
    #     print(i.comment)
    final_rating = round((rating_sum / total_ratings), 2)
    prev_model_rating = Mobiles.objects.get(product_id='MOB4').model_rating
    if rating != 0:
        model_rating = round(((prev_model_rating * total_comments) + rating) / (total_comments + 1), 2)
    else:
        model_rating = prev_model_rating
    obj, created = Mobiles.objects.update_or_create(product_id='MOB4', defaults={'average_customer_rating':final_rating, 'model_rating': model_rating})
    obj.save()
    rating1 = (rate1 / (total_ratings)) * 100
    rating2 = (rate2 / (total_ratings)) * 100
    rating3 = (rate3 / (total_ratings)) * 100
    ...
    final_star = final_rating * 20                          # () * 100 / 5 = () * 20
    model_star = model_rating * 20
    comments = Comments.objects.filter(pid='MOB4')
...
def vivoy20(request):
    ...
    if request.method == "POST":
        ...
        if data and cmt:
            ...
            comment.save()
    fieldname = 'rating'
    rating_count = Comments.objects.filter(pid = 'MOB5').values(fieldname).order_by(fieldname).annotate(the_count = Count(fieldname))
    rate1 = 0
    rate2 = 0
    rate3 = 0
    rate4 = 0
    ...
    rating_sum = rate1 * 1 + rate2 * 2 + rate3 * 3 + rate4 * 4 + rate5 * 5
    total_ratings = rate1 + rate2 + rate3 + rate4 + rate5
    word_comments = Comments.objects.filter(pid = 'MOB5').exclude(comment = 'NULL')
    total_comments = len(word_comments)
    # for i in word_comments:
    #     print(i.comment)
    final_rating = round((rating_sum / total_ratings), 2)
    prev_model_rating = Mobiles.objects.get(product_id='MOB5').model_rating
    if rating != 0:
        model_rating = round(((prev_model_rating * total_comments) + rating) / (total_comments + 1), 2)
    else:
        model_rating = prev_model_rating
    obj, created = Mobiles.objects.update_or_create(product_id='MOB5', defaults={'average_customer_rating':final_rating, 'model_rating': model_rating})
    obj.save()
    rating1 = (rate1 / (total_ratings)) * 100
    rating2 = (rate2 / (total_ratings)) * 100
    rating3 = (rate3 / (total_ratings)) * 100
    ...
    final_star = final_rating * 20                          # () * 100 / 5 = () * 20
    model_star = model_rating * 20
    comments = Comments.objects.filter(pid='MOB5')
